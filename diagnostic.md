...
```scss
$body-base-font-size: h1(base-font-siz, 30%);
```
...
@mixin breakpoint($content) {
  @if map-has-key($breakpoints, $content) {
    @media #{inspect(map-get($breakpoints, $content))} {
      @content;
    }
  } @else {
    @warn "Couldn't find a breakpoint named `#{$content}`.";
  }
```
